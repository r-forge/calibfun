...
%\VignetteIndexEntry{calibFun}
%\VignetteKeywords{calibFun}
%\VignetteDepends{calibFun}
%\VignettePackage{calibFun}
...
the calibFun package and its application to the latter type of problem.
\end{abstract}
...
The package we have developed,  \Rpackage{calibFun}, performs univariate calibration
...
curve. The calibFun package is designed to be useful for both statisticians and 
...
detailed discussion of the model fitting in the \Rpackage{calibFun} package.
...
The \Rpackage{calibFun} package implements two methods for model fitting, 
...
\noindent First load the \Rpackage{calibFun} library
...
most circumstances, for this reason \Rpackage{calibFun} implements the constant CV 
as the VFE).
...
\noindent In the paper by Davidian and Haaland and currently in the \Rpackage{calibFun} package
